...
- `STRIPE_SECRET_KEY` — used in `src/api/stripe.ts` (server functions).
  Also needed **at build time** (the SSR bundle loads the Stripe module
  when prerendering `/`). A missing key now **fails the build loudly**:
  `stripe.ts` throws a clear error if it's unset, and
  `nitro.prerender.failOnError` in `vite.config.ts` turns any prerender
  error into a failed build (exit 1).
...
- [x] Hardening done: `src/api/stripe.ts` throws a clear error when
      `STRIPE_SECRET_KEY` is unset, and `nitro.prerender.failOnError` is
      set in `vite.config.ts` — a build with a missing key now exits 1
      with the explicit error instead of silently skipping the prerender
