...
function animateFly(el: SVGElement, index: number) {
  const char = el.dataset.char as keyof typeof typeMap;
  if (el.ownerSVGElement && el.ownerSVGElement.style.opacity === "0")
    el.ownerSVGElement.style.opacity = "1";
  animate(
    el,
    {
      x: [typeMap[char].x, 0],
...
function handleAnimation(el: SVGElement | null): void {
  if (!el) return;
  const parent = el.parentElement;
  const children = parent ? Array.from(parent.children) : [];
  const index = children.indexOf(el);
  const isPath = Boolean(el.getAttribute("stroke-width"));
  if (isPath) {
  ...
  return (
    <section id="start" class="bg-glb-black h-screen-header" {...props}>
      ...
      >
        ...
        />
        <g ref={handleAnimation} data-char="Ö">
          <path
            fill="var(--color-glb-white)"
            fill-rule="evenodd"
            clip-rule="evenodd"
          ...
          <path
            fill="var(--color-glb-white)"
            d="M130.188 144.759H100.604V137.033H130.188V144.759Z"
          />
        </g>
        <path
          ref={handleAnimation}
        ...
        />
        <g ref={handleAnimation} data-char="LINJEN">
          <path
            fill="var(--color-glb-white)"
            fill-rule="evenodd"
            clip-rule="evenodd"
            d="M177.431 328.191V337.322H153.887V291.67H164.452V328.191H177.431Z"
          />
          <path
            fill="var(--color-glb-white)"
            d="M180.823 291.669H191.519V337.321H180.823V291.669Z"
          />
          <path
            fill="var(--color-glb-white)"
            fill-rule="evenodd"
        ...
        <text fill="transparent">Gröna Linjen Logo Animation</text>
      </svg>
    </section>
  );
}
