<script lang="ts">
	...
	const customTransition = (
		node: SVGGElement | SVGPathElement,
		{ order, type }: { order: number; type: keyof typeof typeMap | 'line' }
	) => {
		if (type === 'line') return draw(node as SVGPathElement, { duration, delay: order * duration });

		const conf = typeMap[type];
		return fly(node, { ...conf, duration, delay: order * duration });
	};
